...
def filtering_logs(logs_doc, file_path):

    with open(logs_doc) as f:
        log_text =  f.read()
    ...
    def parse_logs(log_text):
        ...
        for line in log_text.splitlines():
            profiling_match = PROFILING_SUMMARY_RE.search(line)
            if profiling_match:
                profiling_info.append(profiling_match.group(1))
                

            start_match = PROFILING_START_RE.search(line)
            if start_match:
                profiling_info.append(f"Columns detected: {start_match.group(1)}")

            column_match = COLUMN_PROCESS_RE.search(line)
            if column_match:
                profiling_info.append(f"Column {column_match.group(1)}: {column_match.group(2)}")
